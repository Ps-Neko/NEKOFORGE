...
test("renderPrompt(autonomous): 사용자 goal 이 프롬프트 본문에 주입된다", () => {
  const body = renderPrompt("TASK-001", "implementation-worker", autonomousWorkers, {
    goal: "README 에 설치 방법 한 줄 추가",
    spec: "## who\nself-host 운영자", // goal 과 무관한 기본 SPEC
    autonomous: true
  });
  assert.match(body, /README 에 설치 방법 한 줄 추가/, "autonomous 프롬프트에 사용자 goal 이 들어가야 한다");
});

test("renderPrompt(default): 사용자 goal 이 프롬프트 본문에 주입된다", () => {
  const body = renderPrompt("TASK-001", "implementation-worker", autonomousWorkers, {
    goal: "결제 모듈 리팩터링"
  });
  assert.match(body, /결제 모듈 리팩터링/, "default 프롬프트에도 사용자 goal 이 들어가야 한다");
});

test("renderPrompt: goal 이 없으면 goal 섹션을 만들지 않는다 (하위호환)", () => {
  const body = renderPrompt("TASK-001", "implementation-worker", autonomousWorkers, {
    autonomous: true
  });
  assert.doesNotMatch(body, /## 사용자 목표/, "goal 미지정 시 빈 goal 섹션이 없어야 한다");
});
