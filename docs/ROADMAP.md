...
- **Phase WF/RP 완료 (2026-05-20)** : decision.json schemaVersion 0.4 → 0.5. 당시 CLI는 스물다섯 개였고 후속 auto/UX 포함 현재 26개. 8 worker role + 3 profile + 13 rule pack + 13 skill pack. T-WF 5건 + T-RP 4건 e2e 통과. self-host 11회 통과 (#6 Windows .cmd / #7 WF·RP 자가 검증 / #8 v0.5 외부 검증 대기 / #9 --with-worker-stubs / #10 5라운드 문서 정합 / #11 negative fixture 확장 + cross-rule interference).
...
- **Placeholder rule 7 + WF-3 prototype + 00-first-verdict (2026-05-20)** : deterministic rule 16→25. `src/workers/adapter.ts` interface + shellWorkerAdapterStub (auto-spawn 거부). `examples/00-first-verdict/` 외부 사용자 10분 walkthrough.
- **Placeholder rule 10 추가 (2026-05-20, self-host #12)** : deterministic rule 25→**35** (docs +2: missing-cli-doc / broken-doc-link · release-evidence +3: missing-self-host / missing-migration-note / missing-external-review · api-safety +1: missing-auth-boundary · dependency +1: lockfile-mismatch · frontend +3: missing-focus-state / missing-loading-state / contrast-token). info 등급 6 + warning 4. fixture 정밀화. tests 292 / benchmark 30/30 유지.
...
- ~~모든 rule 의 발화 사례가 memory 에 최소 1건씩 적재.~~ **(달성 — 2026-05-19: 16 rule + 1 gate finding 의 eval-cases 17건 누적. examples/phase-rule-coverage/README.md 의 대응표 참조.)**
